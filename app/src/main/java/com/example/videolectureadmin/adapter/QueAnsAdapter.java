...
public class QueAnsAdapter extends RecyclerView.Adapter<QueAnsAdapter.MyViewHolder> {
    ...
    QAFragment productFragment;

    public QueAnsAdapter(Context context, List<Result> resultList, QAFragment productFragment) {
    ...
    public void onBindViewHolder(@NonNull final MyViewHolder myViewHolder, final int i) {
        ...
        final List<Result> resultList1 = dbHelper.GetAllQuesDataBasedOnQues(resultList.get(i).getQuestion());
            ...
            public void onClick(View v) {
                ...
                if (ansText.length() != 0) {
                    if (Utility.isOnline(context)) {
                        ...
                        serviceCaller.callUploadQuesAnsData(resultList.get(i).getQuestion(), resultList1.get(i).getProductId(), ansText, new IAsyncWorkCompletedCallback() {
                            @Override
                            public void onDone(String workName, boolean isComplete) {
                                dialog.dismiss();
                                if (isComplete) {
                                    if (!workName.trim().equalsIgnoreCase("no")) {
                                        productFragment.gtUsersList();
